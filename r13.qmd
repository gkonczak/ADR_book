...
W pierwszej części rozdziału przedstawiono przykłady dotyczące liczby sklepów i stacji paliw. Odwołano sie wyłącznie do funkcji graficznych dostępnych w pakiecie **bdl**. W drugiej części rozdziału przedstawiono przykłady wykresów dla liczby urodzeń żywych na 1000 ludności i liczby zgonów na 1000 ludności w latach 2002-2024. W tym przypadku odwołano sie do funkcji graficznych z pakietu **ggplot2**.
...
# Wymuś wczytanie zmiennych środowiskowych z pliku .Renviron w katalogu projektu
readRenviron(".Renviron") 

# 
if(Sys.getenv("BDL_API_PRIVATE_KEY") == "") warning("Uwaga: Brak klucza BDL w środowisku renderowania!")

options(bdl.api_private_key = Sys.getenv("BDL_API_PRIVATE_KEY"))
cat("BDL API KEY wczytany: ", substr(Sys.getenv("BDL_API_PRIVATE_KEY"), 1, 15), "...\n")

stopifnot(Sys.getenv("BDL_API_PRIVATE_KEY") != "")
...
Pakiet **bdl** umozliwia wyszukiwanie informacji o dostępnych zmiennych na podstawie określonych słów kluczowych. Poniżej przedstawiono przykład poszukiwania informacji o sklepach z wykorzystaniem funkcji `search_variables()`. 
...
Następnie na podstawie otrzymanych informacji możliwe jest pozyskanie danych w oparciu o funkcję `get_variables()`. Odpowiednie komendy przedstawia poniższy kod.
...
Pobranie danych z Banku Danych Lokalnych o liczbie sklepów i stacji paliw w roku 2024 realizuje następująca komenda

```{r }
sklepy=get_data_by_variable("58964",year="2024",unitLevel=2)
stacje_paliw=get_data_by_variable("58965",year="2024",unitLevel=2)
```
...
pie_plot(data_type ="variable" ,"58965", "2024", unitLevel = "1")
```

Na @fig-paliwo przedstawiono strukturę liczby stacji paliw  według makroregionów w roku 2024. Najwięcej stacji paliw funkcjonuje w makroregionie pólnocno-zachodnim (1601), a najmniej w makroregionie południowo-zachodnim (737).
...
#| fig-width: 12
scatter_2var_plot(data_type = "variable" ,c("58964", "58965"), unitLevel = "2",year=2024)
```
...
Dla przedstawienia zmian w czasie można wykorzystac wykres liniowy, który można utworzyć z wykorzystaniem  dostępnej w pakiecie **bdl** funkcji `line_plot()`.
...
```
Na @fig-sklepy przedstawiono liczbę sklepów w województwach śląskim i małopolskim w latach 2004-2024. W badanym okresie widoczne jest zmniejszanie sie liczby sklepów w obu województwach.
...
W takich przypadkach znacznie ciekawsza jest analiza, gdzie dana zmienna jest przeliczona w stosunku do liczby ludności lub powierzchni danego wojewodztwa.
...
```{r}
#| label: fig-ur_lin
#| fig-cap: Urodzenia żywe na 1000 ludności województwach w latach 2005-2024
#| fig-width: 12
#| fig-height: 8
...
Na @fig-ur_lin widoczny jest systematyczny spadek liczby urodzeń żywych na 1000 ludności we wszystkich województwach w ostatnich latach. Zauważalny jest także znaczny wzrost liczby zgonów na 1000 ludności w latach 2020 i 2021, na co miała wpływ m.in. pandemia COVID-19.


## Porównanie zmiennych w dwóch okresach

Na @fig-ur_zg_wojew przedstawiono liczbę urodzeń żywych i liczbę zgonów w województwach w latach 2008 i 2024. Na wykresach zaznaczono przekątną. Punkty odpowiadające  województwom, w których liczba urodzeń żywych na 1000 ludności jest większa niż liczba zgonów na 1000 ludności znajdują się poniżej przekątnej i oznaczone zostały kolorem zielonym. O ile w roku 2008 w większości województw liczba urodzeń żywych przewyższała liczbę zgonów, to w roku 2024 we wszystkich województwach liczba zgonów była większa od liczby urodzeń żywych.


```{r}
#| label: fig-ur_zg_wojew
#| fig-cap: Urodzenia żywe i zgony na 1000 ludności w województwach w latach 2008 i 2024
#| fig-width: 12
#| fig-height: 6
library(ggrepel)
woj %>% 
  filter(rok==2008 | rok==2024) %>%
...
Do sporządzenia wykresu rozrzutu dla liczby urodzeń żywych i liczby zgonów na 1000 ludności (@fig-02) wykorzystano dodatkową zmienną - liczbę ludności, którą na wykresie reprezentuje wielkość punktów.
...
```
Na @fig-02 przedstawiono liczbę urodzeń żywych na 1000 ludności, liczbę zgonów na 1000 ludności oraz liczbę ludności w województwie w roku 2024. Najwięcej urodzeń żywych na 1000 ludności w roku 2024 było w województwach małopolskim (7,75), mazowieckim (7,66) oraz pomorskim (7,47). Najmniejsze wartości tej zmiennej zanotowano w świętokrzyskim (5,56) i opolskim (5,80). Największa liczba zgonów na 1000 ludności w roku 2024 miała miejsce w województwach łódzkim (12,8) i świętokrzyskim (12,2). Najmniejsze wartości tych zmiennych miały miejsce w  małopolskim (9,57), podkarpackim (9,82) oraz pomorskim (9,94). 

### Kartogram

Poniższy kod prowadzi do uzyskania kartogramu dla liczby urodzeń żywych na 1000 ludności według województw w roku 2024. Efekt widoczny jest na @fig-mapa1.

```{r }
#| echo: true
#| label: fig-mapa1
#| fig-cap: Urodzenia żywe na 1000 ludności według województw w roku 2024
#| fig-width: 8
#| fig-height: 8
...
#| echo: true
#| label: fig-mapa2
#| fig-cap: Urodzenia żywe na 1000 ludności według powiatów w roku 2024
#| fig-width: 8
#| fig-height: 8
...
#| fig-cap: Liczba powiatów według województw w roku 2024
#| width: 10
#| height: 6
pow %>%
  filter(rok==2024) %>%
...
Kolejny kod prowadzi do konstrukcji wykresu rozrzutu dla zmiennych liczba urodzeń żywych na 1000 ludności i liczba zgonów na 1000 ludności według powiatów w latach 2008 i 2024. 

```{r}
#| label: fig-powiaty
#| fig-cap: Urodzenia żywe na 1000 ludności i zgony na 1000 ludności w powiatach w latach 2008 i 2024
#| fig-width: 12
#| fig-height: 6
pow %>%
  filter(rok==2008 | rok==2024) %>%
...
Na @fig-powiaty zauważalne jest, że w roku 2024 w niewielu powiatach liczba urodzeń żywych na 1000 ludności była większa od liczby zgonów na 1000 ludnosci, podczas gdy w roku 2008 takie powiaty stanowiły wyraźną większość.


Zmiana rozkładów badanych zmiennych w latach 2008 i 2024 przedstawiono na wykresach gęstości. 

```{r}
#| label: fig-ur_zg_wojew1
#| fig-cap: Liczba urodzeń żywych na 1000 ludności i liczba zgonów na 1000 ludności w powiatach w latach 2008 i 2024
#| fig-width: 12
#| fig-height: 6
...
p2 <- 
  ...
  scale_y_continuous(
    labels = function(x) format(x, digits = 1) ) +
  xlim(4,17)+
  labs(x="Wskaźnik na 1000 ludności",y="gęstość",title='2024')

p1/p2

```

Urodzenia żywe na 1000 ludności i zgony na 1000 ludności w latach 2008 i 2024 przedstawia @fig-ur_zg_wojew1.  Dla zwiększenia czytelności wykresu oraz w celu zaznaczenia istoty różnic w pojęciach "urodzenie" i "zgon" wykresy gęstości zostały przedstawione ponad osią OX (urodzenia) i pod osią OX (zgony).
...
Poniżej przedstawiono przykład animacji dla liczby urodzeń żywych na 1000 ludności, liczby zgonów na 1000 ludności według województw w latach 2002 - 2024.

Statyczny wykres dla roku 2024 przedstawia @fig-01.

```{r }
#| echo: true
#| label: fig-01
#| fig-cap: Urodzenia żywe na 1000 ludności i zgony na 1000 ludności według województw w roku 2024
#| fig-width: 12
#| fig-height: 8
woj %>% filter(rok==2024) %>%
ggplot(
...
Poniższy kod wprowadza animację dla tych zmiennych dla lat 2002 - 2024.
...
2. Pobierz dane o liczbie ludności ogółem w województwach za rok 2024.

3. Pobierz dane o liczbie urodzeń w powiatach od 2015 do 2024 roku.

4. Stwórz ranking powiatów według liczby studentów w roku 2024.

5. Porównaj PKB per capita w województwach w roku 2024.
