...
**R** to darmowy, open-source'owy język programowania i środowisko obliczeniowe, które jest powszechnie wykorzystywane w analizach statystycznych i wizualizacji danych. Język programowania **R** został skonstruowany przez Rossa Ihakę i Roberta Gentlemana na Uniwersytecie w Auckland w Nowej Zelandii. Prace nad **R** rozpoczęły się na początku lat 90. XX wieku, a pierwsza publiczna wersja została wydana w 1995 roku. Oficjalną wersję 1.0 udostępniono w 2000 roku. **R** jest powszechnie używany zarówno w badaniach akademickich, jak i w przemyśle, dzięki swojej wszechstronności oraz ogromnej społeczności dostarczającej pakiety rozszerzające jego możliwości. W styczniu 2026 roku dostępnych jest ponad 23 tysięcy bibliotek rozszrzających możliwosci programu **R**.
...
W celu zainstalowania programu **R** na stronie internetowej <https://cran.r-project.org/> należy wybrać jedną z wersji **R**. Program jest dostępny dla trzech systemów operacyjnych:
...
Przechodząc do kolejnego okna należy wybrać link **base**, a następnie pobrać (Download) aktualną wersję programu i zainstalować na komputerze.
...
Możliwe jest również wykonywanie innych działań, w tym obliczenia wartości bezwzględnej z liczby, reszty z dzielenia za pomocą operatora `%%`, a także wyznaczenia wartości liczby $\pi$, funkcji m.in. trygonometrycznych (sinus, cosinus, ...) oraz wartości symbolu Newtona.
...
Symbol Newtona $\binom{4}{2}$ - liczba kombinacji 2-u elementowych ze zbioru liczącego 4 elementy wyznaczony jest następująco:
...
::: callout-warning
## Funkcje `ceiling` i `floor` mają tylko jeden argument. Może być to pojedyncza liczba bądź wektor liczbowy, macierz lub ramka danych.
:::
...
2)  W nazwie zmiennej mogą wystąpić cyfry i znak `_`, ale nie na samym początku.
...
::: callout-warning
## Cudzysłowy `" "` oraz `' '` są stosowane zamiennie.
...
Pierwotnie tekst jest wyświetlany w cudzysłowiu, ale można go usunąć za pomocą następującego kodu.
...
Zamiast przeglądać cały zbiór, który często jest zbyt duży do wyświetlenia w jednym oknie, można podejrzeć kilka pierwszych wierszy. Jest to możliwe wywołując komendę `head`. Podając drugi argument (*n*) określa się liczbę wyświetlanych wierszy.
...
Dla sprawdzenia klasy obiektu wykorzystuje się funkcję `class`. 

```{r}
class(cars)
```

Klasa oblektu (`data.frame`) jest inna od dotychczas poznanych.

Sporządzając wykres zbioru danych **cars** za pomocą funkcji `plot` automatycznie tworzony jest wykres rozrzutu, za pomocą którego identyfikować można zależności między zmiennymi.
...
Wykres pudełkowy to wykres niezwykle użyteczny, prezentujący rozkład zmiennej, ale też jej wartości odstające. Zaznaczone są na nim kwartyle, wartość minimalna i maksymalna. Wykres na @fig-wp01 przedstawia rozkład wysokości czarnej czereśni.
...
Macierzowy wykres rozrzutu (scatterplot matrix) to wykresem, który przedstawia w jednym oknie wykresy rozrzutu dla wskazanych zmiennych.
...
::: callout-warning
## Funkcja `barplot` jako zbioru danych wymaga wektora lub macierzy wartości opisujących słupki tworzące wykres. Formuła `chickwts$feed` zawierająca informacje o rodzaju paszy jest zmienną typu `factor`. Zadanie jej wprost do funkcji `barplot` zwróci błąd.
...
Chcąc przedstawić liczbę kurczaków żywionych daną paszą należy najpierw zliczyć w tabeli wystąpienia każdej z pasz. Realizuje to poniższy kod.
...
Wykresy kołowe służą m.in. do zaprezentowania udziału danej grupy w całości. Chcąc zwizualizować jaki jest procent kurczaków spożywających daną paszę należy  ustalić jaki procent całości stanowią konkretne grupy. Służy do tego funkcja `prop.table` zwracająca udział grup w całości. Aby udziały wyrazić w procentach, wartości te mnoży się przez 100.
...
Zbiór **swiss** zawiera dane demograficzne i społeczno-ekonomiczne dla 47 francuskojęzycznych prowincji Szwajcarii z około 1888 roku. Jest to klasyczny zestaw danych wykorzystywany często do analizy wpływu czynników kulturowych i ekonomicznych na dzietność w okresie transformacji demograficznej.
Jest to ramka danych (data.frame) składająca się z 47 obserwacji (wierszy) i 6 zmiennych (kolumn):

- Fertility: Standaryzowana miara płodności.
- Agriculture: Odsetek mężczyzn zatrudnionych w rolnictwie.
- Examination: Odsetek poborowych, którzy uzyskali najwyższą ocenę z egzaminu wojskowego.
- Education: Odsetek poborowych posiadających wykształcenie wyższe niż podstawowe.
- Catholic: Odsetek ludności wyznania katolickiego.
...
Zapisanie pliku w folderze domyślnej lokalizacji.
...
::: callout-warning
## Przy zapisie nie podaje się już ścieżki dostępu. Do funkcji przekazuje się już tylko dane i nazwę pliku z jego rozszerzeniem (w cudzysłowie).
:::
...
W celu zaintalowania programu **R Studio** należy ze strony https://posit.co/download/rstudio-desktop/ wybrać wersję odpowiednią dla systemu operacyjnego dostępnego na swoim komputerze.
