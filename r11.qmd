...
Jeden z wazniejszych pakietów wchodzących w sklad pakietu **tidyverse** jest **dplyr**. Krótka charakterystyka tego pakietu i wybrane przykłady zastosowań zamieszczono poniżej.
...
Poniżej przedstawiono przykłady zastosowania wyżej wyminienionych wymienionych funkcji.
...
dane <- read_csv("http://stat.ue.katowice.pl/dane1/cars.csv")
head(dane,3)
```
...
Pakiet **tibble** oferuje rozszerzenie tradycyjnej klasy danych ramki danych (data.frame). Tibble to nowoczesna wersja data.frame, która poprawia niektóre ograniczenia tej klasy, czyniąc pracę z danymi bardziej intuicyjną i wydajną. Głównym celem pakietu **tibble** jest ułatwienie pracy z danymi.
Najważniejsze funkcje tego pakietu to:
...
Pakiet **forcats** został zaprojektowany dla ułatwienia pracy z danymi jakościowymi, szczególnie z czynnikami (ang. factors). Pakiet dostarcza funkcje, które pozwalają na manipulację, modyfikowanie i porządkowanie zmiennych jakościowych w sposób bardziej intuicyjny i spójny w porównaniu do tradycyjnych operacji na czynnikach w **R**.
Najważniejsze funkcje tego pakietu to:

-   fct_inorder() – tworzenie czynników na podstawie kolejności,
-   fct_reorder() – zmiana porzadku poziomów czynnika na podstawie innej zmiennej,
