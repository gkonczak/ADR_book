...
W zbiorze wyróżniono 11 następujących zmiennych:
...
: Składniki gramatyki grafiki w **ggplot2** {#tbl-gg}
...
Wprowadzając kolor ze względu na wybraną zmienną uzyskuje się możliwość przekazania na wykresie dodatkowych informacji. Takie rozwiązanie przedstawia poniższy kod, a wynik jest przedstawiony na @fig-hist3.
...
Reprezentacje geometryczne często można stosować łącznie. Jeśli do poprzedniego kodu dodana zostanie reprezentacja geometryczna `geom_point()`, to na wykresie poza "pudełkami" zostaną przedstawione również punkty reprezentujące pojedyncze obserwacje (por. @fig-box2).
...
ggplot(mtcars, aes(x=wt, y=mpg)) +
  ...
  facet_wrap(~cyl) 
```
Wykorzystanie warstwy `facet_grid()` pozwala na uwzględnienie dwóch zmiennych przy konstrukcji paneli. Poniższy kod prowadzi do konstrukcji wykresów rozrzutu, gdzie panele są ze względu na liczbę cylindrów (*cyl*) i liczbę biegów (*gear*). Rezultat przedstawia  @fig-pan2.
...
Przykład umieszczenia histogramu w ujęciu w panelach przedstawiono na @fig-pan3.
...
Liczbę przejechanych mil na galonie paliwa względem typu skrzyni biegów w ujęciu paneli ze względu na liczbę cylindrów i biegów przedstawia @fig-pan4.
...
ggplot(mtcars, aes(x=factor(am), y=mpg)) +
  ...
  labs(x='Typ skrzyni biegów: 0 - automatyczna, 1 - ręczna')+
  facet_grid(cyl ~ gear) 
